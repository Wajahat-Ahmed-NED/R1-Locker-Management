...
public class newDetails extends JFrame {
	...
	public void customerPanel() {
		...
		customerDetailsPanel.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
				"Customer Details", TitledBorder.LEADING, TitledBorder.TOP, null, Color.LIGHT_GRAY));
		...
		lockerDetailsPanel.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
				"Locker Details", TitledBorder.LEADING, TitledBorder.TOP, null, Color.LIGHT_GRAY));
		...
		// OperatingInstructionComboBox.setModel(new DefaultComboBoxModel(new String[]
		// {"Singly", "Either or Surviver", "Jointly by all of Us","Jointly by two of
		// Us", "Others"}));
		// OperatingInstructionComboBox.setSelectedIndex(-1);

		modeOfOperationComboBox = new JComboBox();
		modeOfOperationComboBox.setModel(new DefaultComboBoxModel(new String[] { "Singly", "Either or Surviver",
				"Jointly by all of Us", "Jointly by two of Us", "Others" }));
		...
		if (chk == 1) {
			...
			rejectButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					int result = JOptionPane.showOptionDialog(null, panel, "Enter Comments",
					...
					if (result == JOptionPane.OK_OPTION) {
						...
						try {
							...
							connection = java.sql.DriverManager.getConnection("jdbc:db2:WA27389", "db2admin",
									"admin123/?");

							PreparedStatement statement = connection
									.prepareStatement("select * from lockerassigned_tr where LOCKERNUM=?");
							...
							if (result2.next()) {
								...
								if (result3.next()) {
									...
									PreparedStatement selectBalanceStatement = connection
											.prepareStatement(selectBalanceQuery);
									...
									if (balance.next()) {
										...
										JOptionPane.showMessageDialog(null,
												"<html>Your Comment:<br>\t" + comment + "\t</html>");
					...
					if (result == JOptionPane.OK_OPTION) {
						...
						try {
							...
							PreparedStatement statement = connection.prepareStatement(
									"UPDATE lockerassigned_tr SET COMMENTS = ?, NOTIFYUSER = ? WHERE LOCKERNUM = ? ;");
							...
							if (rowsUpdated > 0) {
								...
								UIManager.put("Panel.background", new ColorUIResource(255, 255, 255));
								JOptionPane.showMessageDialog(null,
										"<html>Your Comment:<br>" + comment + "<br>send to User</html>");
		...
		DepositDetailsPanel.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
				"Rent & Security Deposit Details", TitledBorder.LEADING, TitledBorder.TOP, null, Color.LIGHT_GRAY));
		...
		if (chk == 0 | chk == 2) {
			...
			modeOfPaymentComboBox.setModel(
					new DefaultComboBoxModel(new String[] { "Complementary", "Security Deposit", "Yearly Rent" }));
		...
		JButton saveButton = new JButton(
				chk == 1 ? "Authorize" : chk == 2 ? "Update" : chk == 3 ? "Surrender" : "Save");
		saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (chk == 0) {
					...
					// java.sql.Connection connection = null;
					// java.sql.Statement lcl_stmt =null;
					// connection = java.sql.DriverManager.getConnection("jdbc:db2:WA27389",
					// "db2admin", "admin123/?");
					// String deleteQuery = "DELETE FROM lockerassigned_tr WHERE LOCKERNUM=?";
					//
					// PreparedStatement deleteStatement = connection.prepareStatement(deleteQuery);
	...
	public static boolean validateDate(String dateString) {
		...
		try {
			...
			System.out.println(
					"The date you entered is: " + date.getDate() + "  " + date.getMonth() + "  " + date.getYear());
		...
		try {
			...
			if (resultAccount.next()) {
				...
				if (branchCode == 1001) {
					...
					customerAccountRelationship.put("branchcode",
							"Yes - " + Integer.toString(resultAccount.getInt("BRANCHCODE")));
				}
				customerAccountRelationship.put("branchcodeid", Integer.toString(resultAccount.getInt("BRANCHCODE")));
				customerAccountRelationship.put("operatinginstruction",
						resultAccount.getString("OPERATINGINSTRUCTION"));
	...
	public void fetchDetails() {
		try {
			...
			PreparedStatement statement = connection.prepareStatement(
					"select temp.customername,temp.email, temp.contactno,temp.accountNum,temp.lockerno,temp.lockersizeid,temp.keynum, temp.rentstatus,temp.depositamount,temp.overdue, temp.lastrecoverdate, p.modeofpayment,p.expiry_date, o.modeofoperation from modeofpayment p,("
			...
			if (result.next()) {
				...
				if (result.getString("MODEOFOPERATION").equals("Complementary")) {
					...
					//
					// DepositDetailsPanel.setForeground(new Color(0, 0, 0));
					// DepositDetailsPanel.setBorder(new TitledBorder(new
					// EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160,
					// 160, 160)), "Rent & Security Deposit Details", TitledBorder.LEADING,
