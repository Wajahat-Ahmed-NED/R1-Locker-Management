...
//	private JTextField operatingInstructionText;
	...
	public JPanel DepositDetailsPanel = new JPanel();
    ...
    saveButton.addActionListener(new ActionListener() {
    	public void actionPerformed(ActionEvent arg0) {
    		if(chk==0)
    		{
	            ...
	            System.out.println(modeOfPaymentComboBox.getSelectedIndex());
	            if (depositText.getText().length()>0 || 
	            		recoverDateText.getText().length()>0||
	            		overdueDateText.getText().length()>0 ) {
...
public void maintainLocker() {
	...
	try{
		...
		PreparedStatement statement = connection.prepareStatement("update lockerassigned_tl set depositamount=?, lastrecoverdate=?,overdue=? where lockernum=?");
		statement.setString(1, depositText.getText());
		statement.setString(2, recoverDateText.getText());
		statement.setString(3, overdueDateText.getText());
		statement.setString(4, lockerNumber);
		...
		if (result>0) {
			...
			String query2="update voucher_master_tl set modeofoperationid=?,modeofpaymentid=? where voucherid = ?;";
			PreparedStatement statement2 = connection.prepareStatement(query2);
			statement2.setInt(1, modeOfOperationComboBox.getSelectedIndex()+1);
			statement2.setInt(2, modeOfPaymentComboBox.getSelectedIndex()+1);
			statement2.setInt(3, voucherId);
			
			
			System.out.println("haha"+voucherId);
	...
	public void doSurrender() {
		...
		try{
			...
			PreparedStatement statement = connection.prepareStatement("select * from lockerassigned_tl where lockernum=?");
			statement.setString(1, lockerNumber);
			ResultSet result = statement.executeQuery();

			if (result.next()) {
		
				String query2="insert into lockerassigned_tr (voucherid,customerid,depositamount,overdue,lastrecoverdate,rentstatus,assigneddate,comments,notifyuser,lockernum) values (?,?,?,?,?,?,?,?,?,?);";
				PreparedStatement statement2 = connection.prepareStatement(query2);
				statement2.setInt(1, result.getInt("VOUCHERID"));
				statement2.setInt(2, result.getInt("CUSTOMERID"));
				statement2.setInt(3, result.getInt("DEPOSITAMOUNT"));
				statement2.setString(4, result.getString("OVERDUE"));
				statement2.setString(5, result.getString("LASTRECOVERDATE"));
				statement2.setInt(6, 1);
				statement2.setString(7, result.getString("ASSIGNEDDATE"));
				statement2.setString(8, result.getString("COMMENTS"));
				statement2.setInt(9, result.getInt("NOTIFYUSER"));
				statement2.setString(10, result.getString("LOCKERNUM"));
				
				System.out.println("haha"+lockerNumber);
				int result2 = statement2.executeUpdate();
				if(result2>0) {
					JOptionPane.showMessageDialog(null,"Success - Surrender In Process");
					newLockerMaintenance obj=new newLockerMaintenance(3);
					obj.setVisible(true);
					obj.setSize(600, 500);
					dispose();
					...
					JOptionPane.showMessageDialog(null,"Could Not insert in TR");
				}
				
				}
			else {
				JOptionPane.showMessageDialog(null,"Could Not Fetch from TL");
			}
...
//			   
	...
	public void fetchDetails()
	{
		...
		String query="";
		if(chk==3 || chk==2) {
			query="select temp.voucherNo,temp.customername,temp.email, temp.contactno,temp.accountNum,temp.lockerno,temp.lockersizeid,temp.keynum, temp.rentstatus,temp.depositamount,temp.overdue, temp.lastrecoverdate, p.modeofpayment,p.expiry_date, o.modeofoperation from modeofpayment p,("
					+ "select *,tr.lockernum as lockerno,v.voucherid as voucherNo  from lockerAssigned_tl tr, locker l,customer c, voucher_master_tl v where tr.lockernum=l.lockernum and tr.voucherid=v.voucherid and tr.customerid=c.customerid and tr.lockernum=?) temp"
					+ ", modeofoperation o where temp.modeofoperationid=o.modeofoperationid and temp.modeofpaymentid=p.modeofpaymentid;";
		}
		else {
			query="select temp.voucherNo,temp.customername,temp.email, temp.contactno,temp.accountNum,temp.lockerno,temp.lockersizeid,temp.keynum, temp.rentstatus,temp.depositamount,temp.overdue, temp.lastrecoverdate, p.modeofpayment,p.expiry_date, o.modeofoperation from modeofpayment p,("
					+ "select *,tr.lockernum as lockerno ,v.voucherid as voucherNo from lockerAssigned_tr tr, locker l,customer c, voucher_master_tl v where tr.lockernum=l.lockernum and tr.voucherid=v.voucherid and tr.customerid=c.customerid and tr.lockernum=?) temp"
					+ ", modeofoperation o where temp.modeofoperationid=o.modeofoperationid and temp.modeofpaymentid=p.modeofpaymentid;";
		...
		if (result.next()) {
			...
			System.out.println(result.getInt("VOUCHERNO"));
			voucherId=result.getInt("VOUCHERNO");
//			Global.userId=result.getInt("USERID");
			...
			modeOfOperationComboBox.setSelectedItem(result.getString("MODEOFOPERATION"));
			
			System.out.println(result.getString("MODEOFOPERATION"));
			modeOfPaymentText.setText(result.getString("MODEOFPAYMENT"));
			modeOfPaymentComboBox.setSelectedItem(result.getString("MODEOFPAYMENT"));
...
//			
			if(result.getString("MODEOFPAYMENT").equals("Complementary")) {
...
//				DepositDetailsPanel.add(lblNewLabel_3);
        		...
        		if(result.getString("EXPIRY_DATE")!=null) {
    				expiryDateText.setText(result.getString("EXPIRY_DATE"));
    				
    				System.out.println(result.getString("EXPIRY_DATE"));
    			}
    			else {
...
//                 DepositDetailsPanel.repaint();
		...
		{
			dispose();
			JOptionPane.showMessageDialog(null,"Locker Number has Not Found");
	 ...
	 public static void main(String[] args) {
		 newDetails frame = new newDetails(2,"A125");
